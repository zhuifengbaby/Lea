...
namespace LeaRun.Application.Busines.BaseManage
{
    ...
    /// 描 述：用于存储建房审批的记录
    /// </summary>
    public class Load_HomesteadExamineApproveBLL
    {
        private Load_HomesteadExamineApproveIService service = new Load_HomesteadExamineApproveService();
        ...
        /// <returns></returns>
        public void SaveFormADD(string idNumber, Load_HomesteadExamineApproveEntity entity)
        {
            try
            {
                service.SaveForm(idNumber, entity);
            }
            catch (Exception)
            {
                throw;
