...
namespace LeaRun.Application.Service.BaseManage
{
    ...
    /// 描 述：H_RenInfoHis
    /// </summary>
    public class H_RenInfoHisService : RepositoryFactory<H_RenInfoHisEntity>, H_RenInfoHisIService
    {
        #region 获取数据
        /// <summary>
        /// 获取列表
        ...
        public IEnumerable<H_RenInfoHisEntity> GetPageList(Pagination pagination, string queryJson)
        {
            var expression = LinqExtensions.True<H_RenInfoHisEntity>();
            ...
            if (!queryParam["ArearNumber"].IsEmpty())
            {
             ...
             if (!queryParam["ShenName"].IsEmpty())
            {
                string ShenName = queryParam["ShenName"].ToString();
                expression = expression.And(t => t.ShenName.Contains(ShenName));
            }
            ...
            if (queryParam["Status"].ToString() == "2")
            {
                expression = expression.And(t => t.Status == "修改");
            }
            else
            {
                expression = expression.And(t => t.Status != "修改");
        ...
        public IEnumerable<H_RenInfoHisEntity> GetList(string queryJson)
        {
            var expression = LinqExtensions.True<H_RenInfoHisEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
        ...
        /// <returns></returns>
        public H_RenInfoHisEntity GetEntity(string keyValue)
        {
        ...
        /// <returns></returns>
        public void SaveForm(string keyValue, H_RenInfoHisEntity entity)
        {
