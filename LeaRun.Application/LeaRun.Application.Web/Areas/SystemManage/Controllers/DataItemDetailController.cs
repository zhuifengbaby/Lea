...
namespace LeaRun.Application.Web.Areas.SystemManage.Controllers
{
    ...
    public class DataItemDetailController : MvcControllerBase
    {
        ...
        [HttpGet]
        public ActionResult GetDataItemListJson2(string EnCode,string Arear)
        {
            var data = dataItemCache.GetDataItemList2(EnCode,Arear);
            return Content(data.ToJson());
        }
        /// <summary>
        /// 获取数据字典子列表（绑定控件）
        ...
        public ActionResult GetDataItemListJsonAllAddress(string EnCode, string ItemValue)
        {
            var data = dataItemCache.GetSubDataItemListAllAddress(EnCode, ItemValue);
            return Content(data.ToJson());
        }
